...
tuchkaReadyObj.addEventListener('mousedown', (e) => {
    alert ('jshdjshd')
  // Этап 2. Вычисляем начальные координаты положения элемента
  const coords = getCoords(tuchkaReady);
  ...
  const moveAt = (e) => {
    tuchkaReady.style.left = e.pageX - shiftX + 'px';
    tuchkaReady.style.top = e.pageY - shiftY + 'px';
  }
  ...
  tuchkaReady.style.position = 'absolute';
  // Ставим первые значения переноса на точке нахождения элемента
  moveAt(e);
  tuchkaReady.style.zIndex = 1000; // делаем над другими элементами
